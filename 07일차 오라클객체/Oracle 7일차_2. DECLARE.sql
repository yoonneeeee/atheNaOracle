...
-- 1.2. 구조 (익명블록)     - 블록문법
-- 1.2.1. 선언부(선택)         : DECLARE
-- 1.2.2. 실행부(필수)         : BEGIN
-- 1.2.3. 예외처리부(선택)     : EXCEPTION 
-- 1.2.4. END;(필수)           : END;
-- 1.2.5. /(필수)              : /
-- 2. PL/SQL 변수
--------------------------------------------------------------------------------
...
SET SERVEROUTPUT ON;
-- 아래 구문을 그냥 실행하면 나오지 않는다. 실행하기 이전에 반드시 SERVEROUTPUT ON을 해줘야 함
BEGIN 
...
DECLARE
    LAST_NUM EMPLOYEE.EMP_ID%TYPE; -- 마지막 번호 선언
BEGIN
...
BEGIN
    ...
    IF(EMP_INFO.JOB_CODE = 'J1')
    THEN DBMS_OUTPUT.PUT_LINE('저희회사 대표님입니다.');
    ELSE DBMS_OUTPUT.PUT_LINE('일반직원입니다..');
    END IF;
END;
/
--------------------------------------------------------------------------------
...
BEGIN
    ...
    DBMS_OUTPUT.PUT_LINE('급여 : ' || SAL||' 만원');
    DBMS_OUTPUT.PUT_LINE('등급 : ' || SLV||' 등급');
END;
/
--------------------------------------------------------------------------------
-- ELSIF 와 대응되는 CASE문
...
--[범위로 값을 넣을 수 없음]
-- 따라서 나누기 1000000를 해서 1자리만 나오도록 변경

SAL := FLOOR(SAL/10000);
CASE SAL
WHEN 0 ~ 99 THEN SLV := 'F';
WHEN 100 ~ 199 THEN SLV := 'E';
WHEN 200 ~ 299 THEN SLV := 'D';
WHEN 300 ~ 399 THEN SLV := 'C';
WHEN 400 ~ 499 THEN SLV := 'B';
ELSE SLV :='A';
---------------------------------
