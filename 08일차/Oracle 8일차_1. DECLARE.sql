...
-- IF(조건식1) THEN (실행문1) ELSIF (조건식2) THEN (실행문2) 
-- ELSE (실행문)
-- END IF;
--------------------------------------------------------------------------------
-- @실습문제3
...
-- CASE ANS
-- CASE 변수
--      WHEN 변수가 가지는 값 1 THEN 실행문1
--      WHEN 변수가 가지는 값 2 THEN 실행문2
--      WHEN 변수가 가지는 값 3 THEN 실행문3
--      WHEN 변수가 가지는 값 4 THEN 실행문4
...
-- EXCEPTION WHEN NO DATE_FOUND THEN DBMS_OUTPUT.PUT_LINE('노 데이터!');
-- NO_DATA_FOUND
...
BEGIN
    DAN := '&단';
    IF(DAN BETWEEN 2 AND 9) -- 만약 입력받은 DAN이 2~9 사이면, 아래 구문 실행
    THEN 
        WHILE(N<10)
        LOOP
            ...
            N := N + 1;
        END LOOP;
    ELSE DBMS_OUTPUT.PUT_LINE('2~9 사이의 수를 입력하세요.');--IF 구문 아니면, 
    END IF;
END;
/
...
BEGIN
    FOR D IN 1..100
    LOOP
        DBMS_OUTPUT.PUT_LINE(D); --1 ~ 100까지 출력됨
    END LOOP;
END;
/
...
BEGIN
    ...
    LOOP
        ...
        FROM EMPLOYEE
        WHERE EMP_ID = I;
            DBMS_OUTPUT.PUT_LINE('사원번호 : '||EINFO.EMP_ID||', 사원이름 : '||EINFO.EMP_NAME||', 이메일 : '||EINFO.EMAIL);
    END LOOP;
END;
/
--------------------------------------------------------------------------------
...
NO NUMBER
, INPUT_DATA DATE DEFAULT SYSDATE);
        -- 생성한 테이블 조회
        SELECT * FROM KH_NUMBER_TBL;
        -- 정보 입력해보기(반복해야할 구문 확인
        BEGIN
